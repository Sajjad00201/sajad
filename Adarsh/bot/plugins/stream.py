...
@StreamBot.on_message((filters.regex("🔒ورود🔒")) & ~filters.edited, group=4)
async def login_handler(c: Client, m: Message):
    try:
        try:
            ag = await m.reply_text("💎گذرواژه را ارسال کنید.\n\n⭕️برای لغو عملیات دستور /cancel را ارسال کنید\n\n🆔 @King_Network7")
            _text = await c.listen(m.chat.id, filters=filters.text, timeout=90)
            if _text.text:
                textp = _text.text
                if textp=="/cancel":
                   await ag.edit("✅عملیات لغو شد.")
                   return
            else:
                return
        except TimeoutError:
            await ag.edit("📛گذرواژه ارسالی اشتباه است📛")
            return
        if textp == MY_PASS:
            await pass_db.add_user_pass(m.chat.id, textp)
            ag_text = "✅ورود با موفقیت انجام شد."
        else:
            ag_text = "📛گذرواژه ارسالی اشتباه است📛"
...
async def channel_receive_handler(bot, broadcast):
    if MY_PASS:
        ...
        if check_pass == None:
            ...
            return
        if check_pass != MY_PASS:
            await broadcast.reply_text("📛گذرواژه ارسالی اشتباه است📛")
